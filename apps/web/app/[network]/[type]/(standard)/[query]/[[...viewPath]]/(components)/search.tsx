...
export const Search = ({ optionGroups, defaultValue }: Props) => {
  ...
  const [searchBarGlow, setSearchBarGlow] = useState<boolean>(false);
  const toggleSearchBarGlow = () => {
    setSearchBarGlow(!searchBarGlow);
  };
        ...
        <div className={searchBarGlow ? "p-[1px] rounded-lg bg-[#9DAAE2]" : ""}>
          <div className="flex items-center h-[2.125rem] border border-mid-dark-100 shadow-[0px_3px_6px_rgba(42,43,46,0.07),0px_1px_2px_rgba(42,43,46,0.04)] rounded-lg w-full bg-white overflow-hidden">
            ...
            <div className="grid grid-cols-[1fr_auto] items-center grow">
              <input
                ref={searchInput}
                onFocus={toggleSearchBarGlow}
                onBlur={toggleSearchBarGlow}
                onChange={(event: any) =>
              ...
              <button
                onClick={() => {
                  if (searchInput.current)
                    router.push(
                      `/${option}/search/${searchInput.current.value}`
                    );
                }}
