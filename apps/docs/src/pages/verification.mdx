...
Watch [this video tutorial](https://youtu.be/3iPW2PJysGU) and follow the instructions below.
...
It is possible for your contract to be fully or partially verified. Partial verification refers to cases when the deployed bytecode of the onchain contract matches the bytecode resulting from the recompilation with the metadata and the source files _but does not match the metadata hash_.
