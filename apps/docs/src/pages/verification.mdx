...
You must have the .sol contract file, the .json metadata file for the contract in question, and the contract address.
...
To run the development server from the command line, input:
```bash
yarn  dev
```
