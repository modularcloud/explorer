...
function createBaseFeePool(): FeePool {
  return { communityPool: [], decimalPool: [] };
}
...
export const FeePool = {
  encode(message: FeePool, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    for (const v of message.decimalPool) {
  ...
  fromJSON(object: any): FeePool {
    return {
      communityPool: globalThis.Array.isArray(object?.communityPool)
        ...
        : [],
      decimalPool: globalThis.Array.isArray(object?.decimalPool)
        ? object.decimalPool.map((e: any) => DecCoin.fromJSON(e))
        : [],
    };
  },
  ...
  toJSON(message: FeePool): unknown {
    ...
    if (message.decimalPool?.length) {
      obj.decimalPool = message.decimalPool.map((e) => DecCoin.toJSON(e));
    }
    return obj;
  },
  ...
  fromPartial<I extends Exact<DeepPartial<FeePool>, I>>(object: I): FeePool {
    ...
    message.decimalPool = object.decimalPool?.map((e) => DecCoin.fromPartial(e)) || [];
    return message;
  },
};
